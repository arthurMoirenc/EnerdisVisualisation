...
public class MainController {
    ...
    @FXML
    private TableColumn<Donnee, Integer> frequenceCol;

    @FXML
    private TableColumn<Donnee, Integer> tensionCol;

    @FXML
    private TableColumn<Donnee, Integer> puissanceCol;

    @FXML
    private TableColumn<Donnee, Integer> intensiteCol;
    ...
    private void setGraphique(NumberAxis xAxis, NumberAxis yAxis, LineChart<Number, Number> lineChart,
        ...
        List<Donnee> donnees = DonneeDAO.getAllDonnees();
        ...
        for (Donnee donne: donnees) {
            ...
            if (unit == "HZ"){
                valeur = donne.getFrequence();
            }else if (unit == "V"){
                valeur = donne.getTension();
            }else if (unit == "KW"){
                valeur = donne.getPuissance();
            }else if (unit == "A"){
                valeur = donne.getIntensite();
            }

            series.getData().add(new XYChart.Data<>(cur / 60000, valeur));
        }
    ...
    public void initialize() {
        ...
        puissanceCol.setCellValueFactory(new PropertyValueFactory<>("puissance"));
